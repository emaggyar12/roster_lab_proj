...
TABLE_NAME = "evan_miya_barttorvik_same_school_next_year"

OUTPUT_DIR = Path("catboost_transfer_bpr_refactored_outputs")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

TARGET_COL = "next_basic_bpr"
YEAR_COL = "old_year"
...
RAW_NUMERIC_FEATURES = [
    ...
    # Final engineered height feature.
    # Created from old_bvt_ht / old_bvt_player_height.
    "player_height_in",
]

HEIGHT_SOURCE_COLS = [
    "old_bvt_ht",
    "old_bvt_player_height",
]
...
WHERE {qident(YEAR_COL)} IS NOT NULL
  AND {qident(TARGET_COL)} IS NOT NULL
;
"""
...
def parse_height_to_inches(value):
    ...
    # Handles Excel-style strings like ="6-7"
    if s.startswith("="):
        s = s[1:].strip()
...
def prepare_catboost_frame(
    ...
    numeric_cols_to_convert = list(set(numeric_features + [TARGET_COL, YEAR_COL]))
...
def evaluate_split(
    ...
    metrics = {
        ...
        "year_min": float(df_split[YEAR_COL].min()),
        "year_max": float(df_split[YEAR_COL].max()),
...
def main():
    ...
    run_dir = OUTPUT_DIR / f"catboost_transfer_refactored_run_{run_id}"
    ...
    log("Starting refactored CatBoost transfer BPR training")
    ...
    log(f"Rows after target/year filter: {after_filter:,}")
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trial.set_user_attr(
            "valid_mae",
            float(mean_absolute_error(valid_df[TARGET_COL], valid_preds)),
        )
        trial.set_user_attr(
            "valid_r2",
            float(r2_score(valid_df[TARGET_COL], valid_preds)),
        )
        trial.set_user_attr(
            "valid_corr",
            safe_corr(valid_df[TARGET_COL], valid_preds),
        )
        trial.set_user_attr(
            "valid_spearman_corr",
            safe_spearman(valid_df[TARGET_COL], valid_preds),
        )
    ...
    study = optuna.create_study(
        direction="minimize",
        sampler=sampler,
        study_name=f"catboost_transfer_refactored_{run_id}",
    )
    ...
    cbm_path = run_dir / "catboost_transfer_refactored_model.cbm"
    pkl_path = run_dir / "catboost_transfer_refactored_model.pkl"
    ...
    # ----------------------------
    # Production model: train on train + valid
    # ----------------------------
    ...
    production_cbm_path = run_dir / "catboost_transfer_refactored_production_train_valid_model.cbm"
    production_pkl_path = run_dir / "catboost_transfer_refactored_production_train_valid_model.pkl"
    ...
    config = {
        ...
        "split": {
            "method": "chronological cumulative row share by old_year",
